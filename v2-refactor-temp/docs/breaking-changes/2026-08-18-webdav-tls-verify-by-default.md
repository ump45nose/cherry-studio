...
Backup archives and staging directories are additionally written owner-only (0600/0700 — POSIX semantics only; Windows does not enforce these modes). On filesystems that reject `chmod` outright (FAT/exFAT-class mounts), backup and restore abort with a clear error instead of writing under looser permissions.
